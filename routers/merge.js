...
router.get('/', function (req, res, next) {
    ...
    const FlickrRequest = axios.get(`https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=54e758d3ed2d6cd76738e5da2c76557b&tags=${query}&has_geo=1&format=json&nojsoncallback=1&per_page=12&media=photo`);


    axios.all([twitterRequest, weatherRequest,FlickrRequest]) //weatherRequest
        .then(
...
router.get('/api/search', (req, res) => {
    ...
    s3.headObject(params, (err, data) => {
        ...
        if (data == null) {
            axios.all([twitterRequest, weatherRequest , FlickrRequest]) //FilckrRequest]) //weatherRequest
                .then(
                    axios.spread((...responses) => {
